...
describe("Decentratwitter", function () {
  ...
  describe("Setting profiles", async () => {
    it("Should allow users to select which NFT they own to represt", async () => {
      await decentratwitter.connect(user1).mint(URI);
      ...
      // FAIL CASE
      await expect(decentratwitter.connect(user2).setProfile(2)).to.be.revertedWith("Must own the nft you want to select as your profile");
    });
  });
